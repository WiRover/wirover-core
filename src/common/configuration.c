...
const char* get_packet_log_path()
{
    ...
    if(!config || config_lookup_string(config, CONFIG_PACKET_LOG_PATH, &log_path) == CONFIG_FALSE) {
        DEBUG_MSG("failed to read packet-log-path from config file");
    }
...
int get_status_log_enabled()
{
    ...
    int enabled;
    if(!config || config_lookup_int_compat(config, CONFIG_STATUS_LOG_ENABLED, 
                &enabled) == CONFIG_FALSE) {
        DEBUG_MSG("failed to read %s from config file", CONFIG_STATUS_LOG_ENABLED);
        enabled = DEFAULT_STATUS_LOG_ENABLED;
    } else if(enabled != 0 && enabled != 1) {
        DEBUG_MSG("%s %d is not valid", CONFIG_STATUS_LOG_ENABLED, enabled);
        enabled = DEFAULT_STATUS_LOG_ENABLED;
    }
