...
int recvfromClientBurst_timeout(int socket, void* buffer, size_t len, int flags,
        struct timeval *timeout, struct timeval* recvTime);
...
int handleBandwidthClient(const struct bw_server_info* serverInfo, int serverSocket)
{
    ...
    set_timeval_usec(serverInfo->timeout, &timeout);
    ...
    DEBUG_MSG("Bandwidth for node %d link %d down: %f mbps, up: %f mbps, bytes: %d, bits: %d",
...
int handleBandwidthClient_udp(const struct bw_server_info* serverInfo, int sockfd)
{
    ...
    struct timeval timeout;
    timeout.tv_sec = serverInfo->timeout / USEC_PER_SEC;
    timeout.tv_usec = serverInfo->timeout % USEC_PER_SEC;
    ...
    if(gw) {
        ...
        struct interface *ife = find_interface_by_index(gw->head_interface, h_link_id);
        if(ife) {
#ifdef WITH_MYSQL
            db_update_bandwidth(gw, ife, BW_UDP, gw_downlink_bw, gw_uplink_bw);
#endif
        }
    }
...
int recvfromClientBurst_timeout(int socket, void* buffer, size_t len, int flags, struct timeval *timeout, struct timeval* recvTime)
{
    int         result;
    fd_set      readSet;
    sigset_t    sigset;
